...
TEST_CASE("gen::tuple") {
    prop(
        ...
        [](int x1, int x2, int x3) {
            // TODO gen::constant
            const auto gen = gen::tuple(gen::just(x1),
                                           gen::just(x2),
                                           gen::just(x3));
    ...
    prop(
        "shrinks components from left to right",
        [] {
    ...
    prop(
        "passes the right size of each generator",
        [] {
            const int size = *gen::nonNegative<int>();
            const auto result = gen::tuple(genSize(),
                                              genSize(),
                                              genSize())(Random(), size);
    ...
    prop(
        "splits the generator N times and passes the splits from left to right",
        [](const Random &random) {
            const auto result = gen::tuple(genRandom(),
                                              genRandom(),
                                              genRandom())(random, 0);
            Random r(random);
    ...
    prop(
        ...
        [](int v1, int v2) {
            const auto xgen = gen::map(gen::just(v1), [](int x) {
                NonCopyable nc;
                nc.value = x;
                return nc;
            });
            const auto ygen = gen::map(gen::just(v2), [](int x) {
                NonCopyable nc;
                nc.value = x;
                return nc;
            });

    ...
    static void exec()
    {
        templatedProp<T>(
            ...
            [](const Random &random) {
                ...
                const auto shrinkable = gen::container<T>(
                    gen::arbitrary<int>())(random, size);
                const auto value = shrinkable.value();

                const auto i1 = *gen::inRange<std::size_t>(
    ...
    static void exec()
    {
        templatedProp<T>(
            ...
            [](const Random &random) {
                ...
                const auto gen = gen::container<T>(
                    gen::arbitrary<int>(),
                    gen::arbitrary<int>());
...
struct ArbitraryProperties
{
    ...
    static void exec()
    {
        ...
        templatedProp<T>(
            ...
            [] {
                const auto value = gen::arbitrary<T>()(Random(), 0).value();
                RC_ASSERT(
                    std::all_of(
                        begin(value), end(value),
                        [](const Element &x) {
...
TEST_CASE("gen::container(std::size_t)") {
    ...
    prop(
        ...
        [](const GenParams &params) {
            ...
            const auto gen = gen::container<std::array<int, 3>>(

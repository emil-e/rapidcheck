...
Gen<Decay<typename std::result_of<Mapper(T)>::type>> map(
  ...
  return detail::MapGen<T, Decay<Mapper>>(
      std::move(gen), std::forward<Mapper>(mapper));
}
...
Gen<T> cast(Gen<U> gen) {
  return gen::map(
      std::move(gen), [](U &&x) { return static_cast<T>(std::move(x)); });
}
...
Gen<typename std::result_of<Callable(int)>::type::ValueType> withSize(
    Callable &&callable) {
  return [=](
      const Random &random, int size) { return callable(size)(random, size); };
}

...
bool operator==(const Configuration &c1, const Configuration &c2)
{
    return
        (c1.seed == c2.seed) &&
        (c1.maxSuccess == c2.maxSuccess) &&
        (c1.maxSize == c2.maxSize) &&
        (c1.maxDiscardRatio == c2.maxDiscardRatio);
}
...
template<typename T>
bool isNonNegative(T x) { return x >= 0; }
                            ...
                            const Configuration &defaults)
{
    ...
    loadParam(map, "max_size", config.maxSize,
              "'max_size' must be a valid non-negative integer",
              isNonNegative<int>);

    loadParam(map, "max_discard_ratio", config.maxDiscardRatio,

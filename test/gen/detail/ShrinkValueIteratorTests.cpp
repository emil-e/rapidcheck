...
TEST_CASE("ShrinkValueIterator") {
  prop(
      ...
      [](const std::vector<int> &elements) {
        ...
        std::transform(begin(elements),
                       end(elements),
                       std::back_inserter(shrinkables),
                       &shrinkable::just<const int &>);
        std::vector<int> result(makeShrinkValueIterator(begin(shrinkables)),
                                makeShrinkValueIterator(end(shrinkables)));
        RC_ASSERT(result == elements);
      });

  prop("works with non-copyable elements",

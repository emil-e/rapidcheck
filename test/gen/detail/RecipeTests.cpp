...
Shrinkable<int> mapToInt(Shrinkable<Any> shrinkable) {
  return shrinkable::map(
      std::move(shrinkable), [](const Any &x) { return x.get<int>(); });
}
...
TEST_CASE("shrinkRecipe") {
  ...
  prop(
      ...
      [] {
        ...
        auto recipe = *gen::suchThat<Recipe>([](
            const Recipe &r) { return !!shrinkRecipe(r).next(); });

        std::size_t i =
            *gen::suchThat(gen::inRange<std::size_t>(
                               recipe.numFixed, recipe.ingredients.size()),
                [&](std::size_t x) {
        ...
        RC_ASSERT(seq::any(shrinkRecipe(recipe),
            [&](const Recipe &shrink) {

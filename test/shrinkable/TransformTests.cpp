...
TEST_CASE("shrinkable::map") {
  ...
  prop("maps shrinks()",
      [](Shrinkable<int> from) {
        ...
        const auto expected = seq::map(from.shrinks(),
...
TEST_CASE("shrinkable::filter") {
  ...
  prop(
      ...
      [] {
        ...
        RC_ASSERT(shrinkable::all(filtered,
  ...
  prop(
      ...
      [](Shrinkable<int> shrinkable) {
        RC_ASSERT(
            *shrinkable::filter(shrinkable, fn::constant(true)) == shrinkable);
      });
}
...
TEST_CASE("shrinkable::pair") {
  ...
  prop("shrinks first element and then the second one",
      [] {
        ...
        const auto expected = shrinkable::shrinkRecur(std::make_pair(a, b),

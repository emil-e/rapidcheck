...
TEST_CASE("PropertyWrapper") {
    ...
    prop("returns a discard result if a GenerationFailure is thrown",
         [](const std::string &msg) {
             const auto result = makeWrapper([=]{
                 throw gen::GenerationFailure(msg);
             })();
             RC_ASSERT(result.type == CaseResult::Type::Discard);
             RC_ASSERT(result.description == msg);
         });

    prop("returns a failure result with what message if an std::exception is"
         " thrown",
         [](const std::string &msg) {
             const auto result = makeWrapper([=]{
    ...
    prop("returns a failure result with the string as the message if a string"
         " is thrown",
         [](const std::string &msg) {
             const auto result = makeWrapper([=]{
                 throw msg;

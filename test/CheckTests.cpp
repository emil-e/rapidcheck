...
TEST_CASE("checkTestable") {
  ...
  prop("returns the correct number of shrinks on a failing case",
      [] {
        auto evenInteger = gen::scale(0.25,
        ...
        RC_ASSERT(
            failure.numShrinks == ((values.first / 2) + (values.second / 2)));
      });

  prop("returns a correct counter-example",
      [](std::vector<int> values) {
        auto results = checkTestable([&](FixedCountdown<0>, FixedCountdown<0>) {
        ...
        std::transform(begin(values),
            ...
            [](int x) {
              return std::make_pair(typeToString<int>(), toString(x));
            });

...
TEST_CASE("seq::map") {
  ...
  prop("does not copy elements",
      [](std::vector<CopyGuard> elements) {
        ...
        auto mapSeq = seq::map(seq::fromContainer(std::move(elements)), mapper);
        while (mapSeq.next())
          ;
      });
}
  ...
  prop("returns a seq with the elements of all seqs joined together",
      [] {
        ...
        for (const auto &vec : vectors) {
          expectedElements.insert(expectedElements.end(), begin(vec), end(vec));
        }
...
TEST_CASE("seq::concat") {
  prop("joins the given sequences together",
      [](const std::vector<int> &a,
        ...
        RC_ASSERT(
            seq::concat(seq::fromContainer(a),
                seq::fromContainer(b),
                seq::fromContainer(c)) == seq::fromContainer(expectedElements));
      });
...
TEST_CASE("seq::mapcat") {
  prop("equivalent to seq::join(seq::map(...))",
      [](std::vector<int> elements) {
        ...
        RC_ASSERT(seq::mapcat(seq, mapper) == seq::join(seq::map(seq, mapper)));
      });
  ...
  prop("does not copy elements",
      [](std::vector<CopyGuard> elements) {
        ...
        const auto mapper = [](
            CopyGuard &&a) { return seq::just(std::move(a), CopyGuard(1337)); };
...
TEST_CASE("seq::mapMaybe") {
  prop("removes elements for which mapper evaluates to Nothing",
      [](const std::vector<int> &elements) {
        ...
        const auto maybeSeq = seq::mapMaybe(seq,
            [=](int x) -> Maybe<int> {
  ...
  prop("does not copy elements",
      [](std::vector<CopyGuard> elements) {
        auto seq = seq::fromContainer(std::move(elements));
        auto maybeSeq = seq::mapMaybe(std::move(seq),

...
std::string::size_type ParseException::position() const
{ return m_pos; }

std::string ParseException::message() const
{ return m_msg; }
...
bool parsePair(ParseState &s0, std::pair<std::string, std::string> &pair)
{
    ParseState s1(s0);
    skipSpace(s1);
    parseString(s1, pair.first, [] (int c) {
...
std::string maybeQuoteString(const std::string &str, bool doubleQuote)
{
    ...
    bool hasSpecialChar = std::any_of(begin(str), end(str), [] (char c) {
                         ...
                         bool doubleQuote)
{
    if (pair.second.empty()) {
        ...
        return
            maybeQuoteString(pair.first, doubleQuote) + '=' +

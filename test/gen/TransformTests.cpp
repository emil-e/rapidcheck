...
TEST_CASE("gen::map") {
    ...
    prop(
        "uses gen::arbitrary if no generator is specified",
        [](const GenParams &params) {
            const auto value = gen::map<Predictable>(
...
TEST_CASE("gen::suchThat") {
    ...
    prop(
        ...
        [](const GenParams &params) {
            const auto gen = gen::suchThat(gen::just<int>(0),
                                              fn::constant(false));
            const auto shrinkable = gen(params.random, params.size);
            try {
                shrinkable.value();
    ...
    prop(
        ...
        [] {
            ...
            const auto gen = gen::suchThat(genPassedParams(),
                                              fn::constant(true));
    ...
    prop(
        "uses gen::arbitrary if no generator is specified",
        [](const GenParams &params) {
            const auto value = gen::suchThat<Predictable>(
                fn::constant(true))(params.random, params.size).value();
            RC_ASSERT(isArbitraryPredictable(value));
        });

    SECTION("works with non-copyable types") {
        const auto value = gen::suchThat(
            gen::arbitrary<NonCopyable>(),
...
TEST_CASE("gen::withSize") {
    prop(
        ...
        [](const GenParams &params) {
            const auto gen = gen::withSize([](int size) {
